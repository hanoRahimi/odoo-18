...
class AccessManagement(models.Model):
    ...
    hide_menu_ids = fields.Many2many('menu.item', string='Hide Menu')
    hide_field_ids = fields.One2many('hide.field', 'access_management_id', string='Hide Field', copy=True)
    remove_action_ids = fields.One2many('remove.action', 'access_management_id', string='Remove Action', copy=True)
    access_domain_ah_ids = fields.One2many('access.domain.ah', 'access_management_id', string='Access Domain', copy=True)
    hide_view_nodes_ids = fields.One2many('hide.view.nodes', 'access_management_id', string='Button/Tab Access', copy=True)
    
    self_module_menu_ids = fields.Many2many('ir.ui.menu', string='Self Module Menu',
    ...
    # Chatter fields
    hide_chatter_ids = fields.One2many('hide.chatter', 'access_management_id', string='Hide Chatter', copy=True)
    ...
    hide_filters_groups_ids = fields.One2many('hide.filters.groups', 'access_management_id', 
            string='Hide Filters/Group By', copy=True)
