...
class Project(models.Model):
    ...
    def create(self, vals_list):
        ...
        if len(self.env.companies) > 1 and self.env.user.has_group('project.group_project_stages'):
            ...
            stage = self.env['project.project.stage'].search([('name', '=', 'شروع نشده')], limit=1)            # The project's company_id must be the same as the stage's company_id
            if stage.company_id:
                for vals in vals_list:
        ...
        task_types = self.env['project.task.type']._CreateProjectTaskType(projects) # by rh
